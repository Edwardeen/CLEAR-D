...
const HomePage: NextPage = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Section - Varies based on authentication */}
      <section className={`text-white py-20 md:py-32 ${session ? 'bg-gradient-to-r from-blue-600 to-indigo-700' : 'bg-gradient-to-r from-gray-700 to-gray-900'}`}>
        <div className="container mx-auto px-6 text-center">
          {session ? (
            ...
            <>
              <h1 className="text-4xl md:text-6xl font-bold mb-6 animate-fadeInUp">
                Take Control of Your Diabetic Health
              </h1>
              <p className="text-lg md:text-xl mb-10 max-w-3xl mx-auto animate-fadeInUp delay-200">
                CLEAR-D helps individuals with diabetes assess potential risks for Glaucoma and Cancer through early recognition tools.
              </p>
              <div className="flex flex-col items-center space-y-4 sm:flex-row sm:justify-center sm:space-x-4 sm:space-y-0 animate-fadeInUp delay-400">
                <Link href="/login" legacyBehavior>
                  <a className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-8 rounded-lg shadow-md transition duration-300 text-lg transform hover:scale-105 w-full sm:w-auto">
                    Login to Get Started
                  </a>
                </Link>
                <Link href="/register" legacyBehavior>
                  <a className="bg-transparent border-2 border-white text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:bg-white hover:text-gray-800 transition duration-300 text-lg transform hover:scale-105 w-full sm:w-auto">
                    Register Now
                  </a>
                </Link>
              </div>
      ...
      {session && globalStats && !statsLoading && !statsError && (
        <section className="py-16 md:py-24 bg-indigo-50">
          <div className="container mx-auto px-6">
            <SectionTitle title="Community Health Snapshot" subtitle="Global Statistics" />
      ...
      {session && statsLoading && (
          <div className="text-center py-10">
              <FiLoader className="animate-spin h-10 w-10 text-blue-600 mx-auto" />
              <p className="text-gray-600 mt-2">Loading community statistics...</p>
          </div>
      )}
      {session && !statsLoading && statsError && (
        <section className="py-16 md:py-24 bg-red-50">
            <div className="container mx-auto px-6 text-center">
                <SectionTitle title="Statistics Unavailable" subtitle="Community Data" />
                <p className="text-red-600">Could not load community health statistics at this time: {statsError}</p>
            </div>
        </section>
      )}

      {/* Secondary Call to Action Section - Varies or is general */}
      <section className={`py-16 md:py-24 ${session ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'}`}>
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">Ready to Take the Next Step?</h2>
          {session ? (
            <p className="text-lg md:text-xl mb-10 max-w-2xl mx-auto">
              Continue managing your health proactively. Your next assessment awaits.
            </p>
          ) : (
            <p className="text-lg md:text-xl mb-10 max-w-2xl mx-auto">
              Join CLEAR-D today. Register or log in to access personalized assessments and track your health journey.
            </p>
          )}
          <Link href={session ? "/profile" : "/register"} legacyBehavior>
            <a className={`font-semibold py-3 px-8 rounded-lg shadow-md transition duration-300 text-lg transform hover:scale-105 ${session ? 'bg-white text-blue-700 hover:bg-gray-100' : 'bg-white text-green-700 hover:bg-gray-100'}`}>
              {session ? 'Go to Your Profile' : 'Create an Account'}
            </a>
          </Link>
        </div>
      </section>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (!session) {
    // User is not authenticated, return null for session prop
    // The page will then render its public content based on useSession()
    return {
      props: { session: null }, 
    };
  }
  ...
  // If session exists, pass it as a prop
  return {
